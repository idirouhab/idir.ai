...
# Mark all migrations as applied in PRODUCTION without running them
# Use this to initialize the migration tracking for an existing production database
# Usage: SUPABASE_DB_PASSWORD='your_password' ./scripts/mark-all-applied-prod.sh
...
if [ -z "$SUPABASE_DB_PASSWORD" ]; then
    ...
    echo "  ./scripts/mark-all-applied-prod.sh"
    echo ""
    echo "Or run directly:"
    echo "  SUPABASE_DB_PASSWORD='your_password' ./scripts/mark-all-applied-prod.sh"
    exit 1
fi
...
echo -e "${BLUE}=== Mark All Migrations as Applied in PRODUCTION ===${NC}"
echo -e "${YELLOW}Database: $DB_HOST${NC}"
echo ""
echo -e "${RED}⚠ WARNING: This will mark ALL migrations as applied WITHOUT running them${NC}"
echo -e "${YELLOW}Only use this if you know all migrations have already been applied to production${NC}"
echo ""
...
read -p "Mark all $TOTAL migrations as applied in PRODUCTION? (type 'yes' to confirm): " -r
if [[ ! $REPLY =~ ^yes$ ]]; then
...
for file in $MIGRATION_FILES; do
    ...
    IS_APPLIED=$(execute_sql "SELECT COUNT(*) FROM migrations_history WHERE migration_name = '$migration_name';" 2>/dev/null | grep -oE '[0-9]+' | head -1)
