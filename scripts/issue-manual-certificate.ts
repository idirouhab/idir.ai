...
interface ManualCertificateInput {
  ...
  course_title: string;
  course_id?: string; // Optional: if you want to link to a course
...
export async function issueManualCertificate(
  ...
  try {
    ...
    // If course_id not provided, generate a placeholder UUID
    const course_id = input.course_id || crypto.randomUUID();
    ...
    const snapshot = createCertificateSnapshot({
      ...
      course_updated_at: issued_at, // Use issued_at as version since no course record
      completed_at,
      issued_at,
    });
...
if (require.main === module) {
  ...
  if (!name || !email || !courseTitle || !completedAt) {
    ...
    console.log('\nOptional:');
    console.log('  --course-id       Course UUID (generated if not provided)');
